...
export function preCheckSchemes(profile) {
  const { age, gender, income, is_farmer, is_disabled, category, occupation, education, goals } = profile;

  // Convert the goals object (e.g. { "Business": true }) into an array of selected goals
  const selectedGoals = goals && typeof goals === "object"
  ...
  return schemes
    .filter((scheme) => {
      ...
      // Bereavement schemes (e.g., NFBS) — only show if user has a death cert
      if (rules.requires_death_cert === true) {
        const hasDocs = profile.has_documents || {};
        if (!hasDocs.DeathCert) {
          return false;
        }
      }

      // Goal-based Strict Filtering (Option A)
      if (selectedGoals.length > 0) {
        const schemeTags = scheme.tags || [];
        const matchesGoal = schemeTags.some(tag => selectedGoals.includes(tag));
        if (!matchesGoal) {
          return false;
        }
      }
